...
model20 <- function(model, lasi, manifest, biomarker, pca_result, output_name){
  ...
  for(i in 1:n) {
    ...
    formula <- as.formula(paste0(Mname, "~ ", biomarker, "_res + log(r1hagey) + ragender + smoke + r1hmbmi + CD8T + CD4T + NK + Bcell + Mono + PC1 + PC2 + PC3 + PC4 + batch + (1 | Plate) + (1 | row) + offset(log(m_u))"))
