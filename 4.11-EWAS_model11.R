...
model11 <- function(model, lasi, manifest, biomarker, pca_result, output_name){
  ...
  for(i in 1:n) {
    ...
    formula <- as.formula(paste0(Mname, "~ ", biomarker, "_res + log(r1hagey) + ragender + smoke + r1hmbmi + CD8T + CD4T + NK + Bcell + Mono + batch + (1 | Plate)+ offset(log(m_u))"))
    ...
    results[ifelse(i%%n == 0, n, i%%n),] =
      unname(c(gamma_result$coefficients$cond[2,], gamma_result$coefficients$cond[3,],
               ...
               as.numeric(gamma_result$varcor$cond$Plate)))
  }
