...
model4 <- function(model, lasi, manifest, biomarker, pca_result, output_name){
  ...
  lasi[, biomarker] <- log(lasi[, biomarker])  # use log1p(df) if df has no zero values
  lasi[, biomarker] <- scale(lasi[, biomarker])
  lasi[, biomarker] = ifelse(lasi[, biomarker] > 3, 3, ifelse(lasi[, biomarker] < -3, -3, lasi[, biomarker]))
