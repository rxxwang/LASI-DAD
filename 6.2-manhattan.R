...
man = data %>%
  ...
  geom_point(size = 0.1) + 
  geom_hline(yintercept = -log10(threshold), color = "red", linetype = "dashed", linewidth = 0.2) +
  ...
  labs(y = "-log10(p-values)", x = "", title = paste0("Manhattan Plot of ", biomarker)) +
  scale_y_continuous(limits = c(0, 30))+#facet_grid(type ~ method) +
  theme(
...
ggsave(
  plot = manhattan,
  filename = snakemake@output[["manhattan"]],
  width = 10, height = 6
)
