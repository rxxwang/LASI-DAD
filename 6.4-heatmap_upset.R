...
for(i in 1:5){
  ...
  data_all[[i]] = readRDS(snakemake@input[["data"]][i])
...
saveRDS(merged, file = snakemake@output[["pval"]])
...
for(i in 1:5){
  ...
  zscore[[i]] = readRDS(snakemake@input[["data"]][i]) %>% 
    mutate(z = beta/SE) %>% dplyr::select(CpG, z) %>% filter(CpG %in% shared_cpg)
}
...
saveRDS(sig_df, file = snakemake@output[["upset_data"]])
