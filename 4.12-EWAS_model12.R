...
model12 <- function(model, lasi, manifest, biomarker, pca_result, output_name){
  ...
  for(i in 1:n) {
    ...
    formula <- as.formula(paste0(Mname, "~ log(", biomarker, ") + log(r1hagey) + ragender + smoke + r1hmbmi + CD8T + CD4T + NK + Bcell + Mono + batch + offset(log(m_u))"))
